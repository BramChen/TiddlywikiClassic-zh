...
// if (!this.area.id) this.area.id = "area_"+String(Math.floor(Math.random()*10000));
 ...
 this.addButton("h1","жЁ™йЎЊ","'!','',0,1");
// this.addButton("h2","Heading 2","'!!','',0,1");
// this.addButton("h3","Heading 3","'!!!','',0,1");
// this.addButton(" ");
 this.addButton("bold","зІ—й«”","'\\'\\'','\\'\\''");
 this.addButton("italic","ж–њй«”","'//','//'");
 this.addButton("underline","еє•з·љ","'__','__'");
 this.addButton("strike","е€Єй™¤з·љ","'--','--'");
 this.addButton("superscript","дёЉжЁ™","'^^','^^'");
 this.addButton("subscript","дё‹жЁ™","'~~','~~'"); 
// this.addButton(" ");
 this.addButton("ul","жё…е–®","'*','',0,1,1");
 this.addButton("ol","ж•ёе­—жё…е–®","'#','',0,1,1");
// this.addButton(" ");
 this.addButton("indent","зё®жЋ’","'\\t','',0,1");
 this.addButton("outdent","еЏ–ж¶€зј©жЋ’","","document.getElementById('" + this.id + "')._owner.unindent");
 this.addButton("quote","еј•иЁЂ","'>',''");
// this.addButton(" "); 
 this.addButton("hr","е€†йљ”з·љ","'','\\n----\\n',2");
 this.addButton("textred","зґ…е­—","'@@color:red;','@@',2");
 this.addButton("hilitecolor","еј·иЄї","'@@','@@',2");
 this.addButton("createlink","йЌЉзµђ","","document.getElementById('" + this.id + "')._owner.createLink");
 this.addButton("createimage","е…§еµЊењ–жЄ”","","document.getElementById('" + this.id + "')._owner.createImage");
 this.addButton("createtable","жЏ’е…ҐиЎЁж ј","'','\\n|!  |!  A  |!  B  |!  C  |!  D  |\\n|!1|  |  |  |  |\\n|!2|  |  |  |  |\\n|!3|  |  |  |  |\\n',2");
// this.addButton(" ");
 this.addButton("help","иЄЄжЋ","","document.getElementById('" + this.id + "')._owner.help");
/*
...
{
 s =  "WikiEdit 3.01\n";
 s += "		дЅњиЂ…пјљ(c) Roman Ivanov, 2003-2005   \n";
 s += "		з¶ІеќЂпјљhttp://wackowiki.com/WikiEdit \n";
 s += "\n";
 s += "TiddlyWiki иЄћжі•ж”ЇжЏґ\n"
 s += "		дЅњиЂ…пјљ(c) Bram Chen, 2006-2008   \n";
 s += "		з¶ІеќЂпјљhttp://ptw.sf.net \n";
 s += "\n";
 s += "еї«йЂџйЌµе€—иЎЁ:\n";
 s += " Ctrl+B - зІ—й«”\n";
 s += " Ctrl+I - ж–њй«”\n";
 s += " Ctrl+U - еє•з·љ\n";
 s += " Ctrl+Shift+S - е€Єй™¤з·љ\n";
 s += " Ctrl+Shift+1 .. 4 - жЁ™йЎЊ 1..4\n";
 s += " Alt+I or Ctrl+T - зё®жЋ’\n";
 s += " Alt+U or Ctrl+Shift+T - еЏЌзё®жЋ’\n";
 s += " Ctrl+J - жЁ™иЁ (@@)\n";
 s += " Ctrl+H - жЁ™иЁ (??)\n";
 s += " Alt+L - йЌЉзµђ\n";
 s += " Ctrl+L - йЌЉзµђиЄЄжЋ\n";
 s += " Ctrl+Shift+L - еЏ–ж¶€жё…е–®\n";
 s += " Ctrl+Shift+N - ж•ёе­—жё…е–®\n";
 s += " Ctrl+Shift+O - жё…е–®\n";
 s += " Ctrl+= - е°Џе­—\n";
 s += " Ctrl+Shift+Minus - е€†йљ”з·љ\n";
 s += " иЄЄжЋдє‹й …: еї«йЂџйЌµжњЄж”ЇжЏґOpera.\n";
 alert(s);
}