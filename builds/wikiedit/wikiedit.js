...
// this.addButton(" ");
 ...
 this.addButton("superscript","Superscript","'^^','^^'");
 this.addButton("subscript","Subscript","'~~','~~'"); 
// this.addButton(" ");
 this.addButton("ul","List","'*','',0,1,1");
 this.addButton("ol","Numbered list","'#','',0,1,1");
// this.addButton(" ");
 this.addButton("indent","Indent","'\\t','',0,1");
 this.addButton("outdent","Outdent","","document.getElementById('" + this.id + "')._owner.unindent");
 this.addButton("quote",">","'>',''");
// this.addButton(" "); 
 this.addButton("hr","Line","'','\\n----\\n',2");
 this.addButton("textred","Textred","'@@color:red;','@@',2");
 this.addButton("hilitecolor","Highlight","'@@','@@',2");
 this.addButton("createlink","Hyperlink","","document.getElementById('" + this.id + "')._owner.createLink");
 this.addButton("createimage","Imagelink","","document.getElementById('" + this.id + "')._owner.createImage");
...
/*
 fIn &&
  onNewLine==0 //добавлU+00FF.ем таги.
  onNewLine==1 //добавлU+00FF.ем таги, если первU+00E0.U+00FF. строка
  onNewLine==2 //добавлU+00FF.ем таги, если первU+00E0.U+00FF._и_последU+00ED.U+00FF.U+00FF. строка, иначU+00E5.
   //добавлU+00FF.ем первый таU+00E3., если первU+00E0.U+00FF. либо добавлU+00FF.ем последниU+00E9., если последU+00ED.U+00FF.U+00FF.
 //иначU+00E5. добавлU+00FF.ем неизменный тексU+00F2.
*/
  if (fIn && (onNewLine==0 | (onNewLine==1 && add==0) | (onNewLine==2 && (add==0 || fOut)))) {
  //добавлU+00FF.ем таги
    if (expand==1) {
      l = lines[i];
...
/*
  не экспанU+00E4.. этU+00EE. значит, чтU+00EE.
  если первU+00E0.U+00FF. строка, то добавлU+00FF.ем реплейсоU+00EC. первый U+00E8. суммой второй
  если последU+00ED.U+00FF.U+00FF., то добавлU+00FF.ем суммой первый U+00E8. реплейсоU+00EC. второй
  если первU+00E0.U+00FF. U+00E8. последU+00ED.U+00FF.U+00FF., то обU+00E0. реплейсоU+00EC.
  иначU+00E5. суммой
*/
//    alert(lines[i].replace(new RegExp("\n","g"),"|").replace(new RegExp(" ","g"),"_"));
//    alert(lines[i+1].replace(new RegExp("\n","g"),"|").replace(new RegExp(" ","g"),"_"));
  ...
  //добавлU+00FF.ем неизменный тексU+00F2.
    r += lines[i];
  }
  if (fOut)
   fIn = false;
...
{
 s =  "         WikiEdit 3.01 for TiddlyWiki\n";
